...
| **Capability**               | O-Cloud **StarlingX**   | O-Cloud **Redhat OKD**       | **Vanilla Kubernetes** (Testing and Dev. Environment)    | Interaction with Proposed Module (NFO and FOCOM) (They run on SMO)
| ----------------             | ---------------         | -------------------          | ------------------------  | -------------------------------------------                                                                                                                                                                                                                    |
| **O2-IMS Interface**         | Native oran-o2 IMS      | oran-o2ims operator on RHACM | None                      | • FOCOM queries O2-IMS for resource inventory<br/>• Requests resource allocation via ProvisioningRequest<br/>• Subscribes to resource change notifications<br/>• Federates orchestration across multiple O-Clouds                                        |
| **O2-DMS Interface**         | Native K8s profile      | oran-o2ims DMS on RHACM      | None                      | • NFO queries available DeploymentManagers<br/>• Requests NF instantiation via O2-DMS<br/>• Provides NF descriptor and deployment parameters<br/>• Monitors NF deployment lifecycle status                                                             |
| **Multi-Cluster Management** | Distributed cloud       | RHACM hub-spoke              | Single Cluster            | • FOCOM discovers O-Clouds via O2-IMS registration<br/>• NFO selects target DeploymentManager per test scenario<br/>• Enables federated resource orchestration<br/>• Parallel NF deployments across sites                                                    |
| **Resource Allocation**      | Platform resource pools | RHACM managed clusters       | K8s nodes/namespaces      | • FOCOM requests compute/storage/network via O2-IMS<br/>• Specifies resource requirements for test workloads<br/>• Receives resource pool allocations<br/>• Tracks capacity across federated O-Clouds                                                      |
| **NF Deployment**            | Helm via DMS            | Helm/operators via DMS       | Direct K8s API            | • NFO sends deployment request to O2-DMS<br/>• Includes NF descriptor with helm charts and values<br/>• O-Cloud DMS instantiates CNF on allocated resources<br/>• Returns deployment status and endpoints                                                 |
| **Monitoring**               | Platform exporters      | Prometheus operator          | Manual setup              | • NFO queries O2-DMS for NF performance metrics<br/>• FOCOM queries O2-IMS for infrastructure health<br/>• Collects test results via exposed metrics endpoints<br/>• Aggregates data for CI/CD regression analysis upon repated testing variations performed |
